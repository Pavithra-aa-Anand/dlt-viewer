...
Explicit accessor APIs used by consumers:

- `messageIdForFilteredRow(int filteredRow)` for filtered-row -> `MessageId` lookup.
- `messageIdForGlobalIndex(int globalIndex)` for global-index -> `MessageId` lookup.
- `globalIndexForMessageId(MessageId id)` for reverse mapping back to navigation rows.
...
- MainWindow filter helpers now use `messageIdForFilteredRow(...)` and
    `globalIndexForMessageId(...)` on the shared `CMessageStore` surface.
...
- `qdlt/qdltmsg.cpp`
- `doc/Single_Data_Model_Design_Outline.md`
