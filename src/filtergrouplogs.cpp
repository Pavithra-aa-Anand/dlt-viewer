...
QStringList CFilterGroupLogs::extractEcuIds(const QString& dltFilePath) {
    ...
    if (m_decodeCacheService && m_dltFile) {
        QSet<QString> uniqueEcuIds;
        const int total = m_dltFile->size();
        const bool decodeEnabled = false;
        const int triggeredByUser = 0;
        QDltMsg msg;
        for (int i = 0; i < total; ++i) {
            if (!m_decodeCacheService->message(m_dltFile,
                                               m_pluginManager,
                                               i,
                                               decodeEnabled,
                                               triggeredByUser,
    ...
    QDltFile m_dltFile;
    QSet<QString> uniqueEcuIds;
    if (!m_dltFile.open(dltFilePath)) {
        return QStringList();
    }
    if (!m_dltFile.createIndex()) {
        m_dltFile.close();
        return QStringList();
    }
    CDecodeCacheService localDecodeCacheService;
    CDecodeCacheService *decodeService = m_decodeCacheService ? m_decodeCacheService : &localDecodeCacheService;
    for (int i = 0; i < m_dltFile.size(); i++) {
        QDltMsg msg;
        const bool decodeEnabled = false;
        const int triggeredByUser = 0;
        const bool ok = decodeService->message(&m_dltFile,
                                               m_pluginManager,
                                               i,
                                               decodeEnabled,
                                               triggeredByUser,
...
void CFilterGroupLogs::ecuIdTabs(){
    ...
    for (int sourceRow = 0; sourceRow < static_cast<int>(filteredProjection.size()); ++sourceRow) {
        ...
        if (m_decodeCacheService) {
            gotMessage = m_decodeCacheService->message(m_dltFile,
                                                       m_pluginManager,
                                                       globalIndex,
                                                       decodeEnabled,
                                                       triggeredByUser,
